/*Max Grubb 12/6/2015
 * Written by OSU CS Staff and Max Grubb
...
{
	struct cirListDeque *r = malloc(sizeof(struct cirListDeque));
	...
	source->isVisited = 1;
	addFrontCirListDeque(c, source);	
	while(!(isEmptyCirListDeque(c))){
		
		v = frontCirListDeque(c);
		if(isAdjacent(v, destination)){
			return 1; 
		}
		else{	
			...
			if(v->isVisited == 0){  
					...
					addFrontCirListDeque(c, v->neighbors[i]);
				}	
			}
		}

	}		return 0;
/*	if(DFSRecursive(g, source, destination))
		return 1; 
	else
		return 0; 	
*/
