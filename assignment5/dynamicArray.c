...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{
	...
	struct TaskP* tsk1 = heap->data[i];
	struct TaskP* tsk2 = heap->data[j]; 

	if(compare(tsk1, tsk2) == -1)
	{	return i; }
//	else if(compare(tsk1, tsk2) == 1)
//	{	return j;} 
//	return 0; 
	else
	{ return j; }
...
{
	struct TaskP* min = getDynArr(heap, 0);
	return min;
...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
	...
	{
		parent = (pos - 1) / 2;
		struct TaskP* left = getDynArr(heap, parent); 
		struct TaskP* right = getDynArr(heap, pos); 
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
	...
	max = sizeDynArr(heap); 
	int parent = (pos - 1) / 2;
///	struct TaskP* leftT = getDynArr(heap, parent);
 	struct TaskP* posT = getDynArr(heap, pos);
 	struct TaskP* parT = getDynArr(heap, parent);
	struct TaskP* smallT; 
	struct TaskP* smallC;  
	...
	if (right < max)
	{
		...
		smallT = getDynArr(heap, small); 
		if (compare(smallT, posT) == -1)
		{
	...
	{
		smallC = getDynArr(heap, left);
		if (compare(smallC, parT) == -1)
		{
			swapDynArr(heap, parent, left);
...
void _buildHeap(DynArr *heap, comparator compare)
{
	...
	int i; 
	for(i = max/2 - 1; i>=0; i--){
		_adjustHeap(heap, max-1, i, compare);
	}
}
/*
...
void sortHeap(DynArr *heap, comparator compare)
{
	...
	swapDynArr(heap,q, 0);
	_adjustHeap(heap, q, 0, compare); 

	for (int p = q-1; p >=0; p--){
		swapDynArr(heap,0,p);
		_adjustHeap(heap, p, 0, compare); 
	}	
